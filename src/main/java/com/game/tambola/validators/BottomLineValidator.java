...
public class BottomLineValidator implements ClaimValidator {
	...
	public ClaimResult validate(TambolaTicket tambolaTicket, List<Integer> announcedNumbers) {
		...
		Set<Integer> rowNumbers = tambolaTicket.getRowNumber(2);
		boolean isValid = announcedNumbers.containsAll(rowNumbers);
		return new ClaimResult(isValid, isValid ? "Bottom Line completed" : "Bottom Line rejected");
	}
